...
def generate_gillespie_model(config_path, intermediates, output_path):
    ...
    for label in ["HM", "HT"]:
        ...
        for i in reversed(intermediates):
            p = 0 if i < 15 else 1 / (10 ** next((x for min_i, max_i, x in dloop_powers if min_i <= i < max_i), 0))
...
if __name__ == "__main__":
    intermediate_complexes = [8] + [i for i in range(9, 51, 3)] + [96, 144, 192, 240, 384]
    path = "params.yaml"
    model_str, species_mapping = generate_gillespie_model(path, intermediate_complexes, "model.txt")
